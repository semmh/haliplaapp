...
page_icon=“🧶”,
layout=“wide”,
initial_sidebar_state=“expanded”,     # üretim parametreleri varsayılan açık
)
...
# Yalnızca tüm grafik türleriyle (Bar, Scatter, Pie) güvenle paylaşılan ayarlar.

# plot_bgcolor : Pie/Sunburst grafiklerinde bazı Plotly versiyonlarında TypeError

# fırlatır — buraya KONULMAZ, yalnızca kartezyen grafiklerde kullanılır.

# margin       : Her grafik kendi margin’ini açık olarak belirtir; burada

# bırakılırsa **kwargs merge sırasında sessiz override çakışması olur.
...
# Bar / Scatter grafiklerde kullanılan ek ayarlar — Pie chart’larda kullanılmaz.

PLOTLY_LAYOUT_CARTESIAN = dict(
**PLOTLY_LAYOUT_BASE,
plot_bgcolor=”#f8f9fa”,
)
...
fig.update_layout(
**PLOTLY_LAYOUT_BASE,           # Pie-safe: plot_bgcolor içermez
title=dict(text=f”⏱️ Süre Dağılımı ({s.sure.saat:,.1f} saat)”,
x=0.5, font=dict(size=14, color=PALETTE[“koyu_mavi”])),
height=340,
margin=dict(t=60, b=10, l=10, r=10),  # explicit — BASE’deki margin yok
showlegend=True,
...
x=0.5, font=dict(size=14, color=PALETTE[“koyu_mavi”])),
height=340,
margin=dict(t=60, b=10, l=10, r=10),  # explicit
)
return fig
...
# px.bar kendi title dict'ini set eder; tekrar title dict geçmek çakışır.
# Güvenli yol: sadece ek layout ayarlarını geç, title'a dokunma.
fig.update_layout(
    **PLOTLY_LAYOUT_CARTESIAN,
    title_x=0.5,
    height=300,
...
with col_r:
    ...
    fig_oran.update_layout(**PLOTLY_LAYOUT_CARTESIAN, showlegend=False, height=260,
